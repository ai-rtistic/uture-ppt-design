...
## 0-1-1. 줄바꿈 검증 루프 (시각 검증 PASS 후 필수)

시각 검증이 PASS 된 뒤, **한국어 줄바꿈 품질**을 별도로 점검한다. 어절(eojeol)이 라인 사이에서 잘리는 문제는 시각 검증 루브릭과 다른 차원이므로 분리해서 본다.

1. **캡처 재사용 또는 재생성** — 0-1 단계에서 만든 PNG를 그대로 사용. CSS·텍스트가 바뀌었다면 재캡처.
2. **검증 호출** — `line-break-verifier` 서브에이전트를 호출한다. 입력은 HTML 경로 + 캡처 PNG 경로 + (선택) 의심 슬라이드 번호.
3. **판정 처리**
   - `PASS` → 사용자에게 인계.
   - `FAIL` → `Required Fixes`만 반영해 수정 (대개 컨테이너 폭 조정, `<br/>` 위치 이동, 또는 `colors_and_type.css` 의 `word-break` 누락 보강) → 1번부터 다시. **최대 3회**.
   - 3회 초과 시 자체 수정 중단 → 검증자 리포트를 사용자에게 보고.
4. **루브릭 출처** — 판정 기준은 [docs/line-break-rubric.md](docs/line-break-rubric.md). 디자인 시스템의 `word-break: keep-all` 정책은 [uture-design-system/README.md](uture-design-system/README.md) § 5 Visual Foundations · 타입 참고.

> 본 루프는 시각 검증과 독립적으로 동작한다. 시각 검증을 통과해도 어절이 잘리고 있으면 인계 전에 반드시 수정한다.
